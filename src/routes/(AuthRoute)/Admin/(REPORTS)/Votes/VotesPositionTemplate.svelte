<script lang="ts">
	import { adminCandidatesArray } from "$lib";
import type { POSITION_TB } from "$lib/types";
	import VotesCandidateTemplate from "./VotesCandidateTemplate.svelte";

    export let position: POSITION_TB

    const {id, description, max_votes } =position;
</script>
...
<div class=" border-slate-500 dark:border-pink-500 ">
    ...
    <div class=" flex flex-col gap-2 max-h-[55dvh] overflow-auto p-2">
                ...
                <div class="shadow overflow-hidden border-b border-blue-200">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <thead class="bg-white dark:text-pink-500 dark:bg-black">
                            ...
                            <tr>
                                <th class="px-6 py-3 text-left w-full ">
                        ...
                        <tbody class="bg-opacity-50 divide-y divide-gray-200 bg-white dark:bg-[#00000050]">                           
                            ...
                            {#each $adminCandidatesArray?.filter(item => item.position_id === id) ?? [] as candidate}
                                <VotesCandidateTemplate {candidate} />
                            {/each}
